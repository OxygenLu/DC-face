...
class CLIP_Adapter(nn.Module):
    # map_num :M_Blk_num
    def __init__(self, in_ch, mid_ch, out_ch, G_ch, CLIP_ch, cond_dim, k, s, p, map_num, CLIP,hidden_size):
        super(CLIP_Adapter, self).__init__()
        self.CLIP_ch = CLIP_ch
        self.FBlocks = nn.ModuleList([])
        self.FBlocks.append(M_Block_RAT(in_ch, mid_ch, out_ch, cond_dim, k, s, p,hidden_size))# 加了rat
        for i in range(map_num-1):
            self.FBlocks.append(M_Block_RAT(out_ch, mid_ch, out_ch, cond_dim, k, s, p,hidden_size))# 加了rat
        self.conv_fuse = nn.Conv2d(out_ch, CLIP_ch, 5, 1, 2)
        self.CLIP_ViT = CLIP_Mapper(CLIP)
        self.conv = nn.Conv2d(768, G_ch, 5, 1, 2)
        #
        self.fc_prompt = nn.Linear(cond_dim, CLIP_ch*8)# 全联接层
...
class NetG(nn.Module):
    def __init__(self, ngf, nz, cond_dim, imsize, ch_size, mixed_precision, CLIP):
        ...
        self.mapping = CLIP_Adapter(self.code_ch, self.mid_ch, self.code_ch, ngf*8, self.CLIP_ch, cond_dim+nz, 3, 1, 1, 4, CLIP,hidden_size=4)
        # build GBlocks
...
class NetD(nn.Module):
    def __init__(self, ndf, imsize, ch_size, mixed_precision):
        ...
        self.cot_attention = CoTAttention(dim=768, kernel_size=3)
...
class M_Block(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, cond_dim, k, s, p):
        ...
        self.conv1 = Conv_Att(in_ch, mid_ch, k, s, p)
        self.fuse1 = DFBLK(cond_dim, mid_ch)
        self.conv2 = Conv_Att(mid_ch, out_ch, k, s, p)
...
class M_Block_RAT(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, cond_dim, k, s, p , hidden_size):
        ...
        self.conv1 = Conv_CoT(in_ch, mid_ch, k, s, p)
        self.fuse1 = DFBLK_rnn(cond_dim, mid_ch, hidden_size)
        self.conv2 = Conv_CoT(mid_ch, out_ch, k, s, p)
...
class G_Block(nn.Module):
    def __init__(self, cond_dim, in_ch, out_ch, imsize):
        ...
        self.imsize = imsize
        self.learnable_sc = in_ch != out_ch 
        self.c1 = Conv_CoT(in_ch, out_ch, 3, 1, 1)
        self.c2 = Conv_CoT(out_ch, out_ch, 3, 1, 1)
...
#DFBLK中的affine+gru
class DFBLK_gru(nn.Module):
    def __init__(self, cond_dim, in_ch, hidden_size):
        super(DFBLK_gru, self).__init__()
        self.gru0 = nn.GRU(in_ch, hidden_size, batch_first=True)
        self.affine0 = Affine(cond_dim, hidden_size)
        self.gru1 = nn.GRU(hidden_size, hidden_size, batch_first=True)
        self.affine1 = Affine(cond_dim, hidden_size)

    def forward(self, x, y=None):
        print(x.shape) # torch.size([16,32,7,7])
        _, (h, c) = self.gru0(torch.zeros(1, x.size(0), self.rnn.hidden_size).to(x.device))
        # h[num_layer, 16, hidden]

        r, _ = self.gru1(h) # short cut r[num_layer, 16, hidden]
        h = x.view(h.size(1), x.size(1), x.size(2), x.size(3)) # h->4D
        r = x.view(r.size(1), x.size(1), x.size(2), x.size(3)) # r->4D
...
class Affine_rnn(nn.Module):
    def __init__(self, cond_dim, num_features, hidden_size):
        super(Affine_rnn, self).__init__()
        self.rnn2 = nn.GRU(cond_dim, hidden_size, batch_first=True)
        self.fc_gamma = nn.Sequential(nn.Linear(hidden_size, num_features),
                                       nn.ReLU(inplace=True),
                                       nn.Linear(num_features, num_features))
        self.fc_beta = nn.Sequential(nn.Linear(hidden_size, num_features),
                                      nn.ReLU(inplace=True),
                                      nn.Linear(num_features, num_features))
        self.residual_conv = nn.Sequential(nn.Conv2d(num_features, num_features, kernel_size=1, stride=1),
                                            nn.ReLU(inplace=True),
                                            nn.Conv2d(num_features, num_features, kernel_size=1, stride=1))
        self._initialize()
    ...
    def forward(self, x, y=None):
        if y is not None:
            _, (h, c) = self.rnn2(y.unsqueeze(0))
        else:
            _, (h, c) = self.rnn2(torch.zeros(1, x.size(0), self.rnn.hidden_size).to(x.device))     
        ...
        residual = x
        x = weight * x + bias
        x = x + self.residual_conv(residual)  # 添加残差连接
        return x
...
#CoT卷积
class Conv_CoT(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv_CoT, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
